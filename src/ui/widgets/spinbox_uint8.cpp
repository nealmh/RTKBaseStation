...
SpinboxUInt8::SpinboxUInt8(lv_obj_t* parent, const char* label) {
...
void SpinboxUInt8::setRange(uint8_t min, uint8_t max) {
  lv_spinbox_set_range(_spinbox, min, max);
  _min = min;
  _max = max;
}

void SpinboxUInt8::setMax(uint8_t max) {
  _max = max;
  lv_spinbox_set_range(_spinbox, _min, max);
}

void SpinboxUInt8::setMin(uint8_t min) {
  _min = min;
  lv_spinbox_set_range(_spinbox, min, _max);
}

void SpinboxUInt8::setValue(uint8_t val) {
  lv_spinbox_set_value(_spinbox, val);
}

uint8_t SpinboxUInt8::getValue(){
    return static_cast<uint8_t>(lv_spinbox_get_value(_spinbox));
}
