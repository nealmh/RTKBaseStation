...
SpinboxUInt16::SpinboxUInt16(lv_obj_t* parent, const char* label) {
...
void SpinboxUInt16::setRange(uint16_t min, uint16_t max) {
  lv_spinbox_set_range(_spinbox, min, max);
  _min = min;
  _max = max;
}

void SpinboxUInt16::setMax(uint16_t max) {
  _max = max;
  lv_spinbox_set_range(_spinbox, _min, max);
}

void SpinboxUInt16::setMin(uint16_t min) {
  _min = min;
  lv_spinbox_set_range(_spinbox, min, _max);
}

void SpinboxUInt16::setValue(uint16_t val) {
  lv_spinbox_set_value(_spinbox, val);
}


uint16_t SpinboxUInt16::getValue(){
    return static_cast<uint16_t>(lv_spinbox_get_value(_spinbox));
}
