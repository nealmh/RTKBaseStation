...
SpinboxInt8::SpinboxInt8(lv_obj_t* parent, const char* label) {
...
void SpinboxInt8::setRange(int8_t min, int8_t max) {
  lv_spinbox_set_range(_spinbox, min, max);
  _min = min;
  _max = max;
}

void SpinboxInt8::setMax(int8_t max) {
  _max = max;
  lv_spinbox_set_range(_spinbox, _min, max);
}

void SpinboxInt8::setMin(int8_t min) {
  _min = min;
  lv_spinbox_set_range(_spinbox, min, _max);
}

void SpinboxInt8::setValue(int8_t val) {
  lv_spinbox_set_value(_spinbox, val);
}

int8_t SpinboxInt8::getValue(){
  return static_cast<int8_t>(lv_spinbox_get_value(_spinbox));
}
